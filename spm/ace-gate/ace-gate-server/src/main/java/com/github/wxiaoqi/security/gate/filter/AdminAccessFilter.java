    ...
    public Object run() {
          ...
          if (permissions.length > 0) {
            ...
            try {
              ...
              Map<String,Object> usermap=userService.userinfoByUserid(user.getId());
              if("1".equals(usermap.get("status").toString())){
                if("-1".equals(usermap.get("userstatus"))){
                  setFailedRequest(JSON.toJSONString(ObjectRestResponse.error(RestCodeConstants.EX_USER_FORBIDDEN_CODE,"此帐号已被冻结!")), HttpStatus.UNAUTHORIZED.value());
              ...
              //更改为统一的返回信息
              setFailedRequest(JSON.toJSONString(ObjectRestResponse.error(RestCodeConstants.EX_USER_INVALID_CODE,"用户身份过期,请重新登录!")), HttpStatus.UNAUTHORIZED.value());
